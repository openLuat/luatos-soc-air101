...
int tls_lsd_recv(u8 *buf, u16 data_len)
{
	...
	if(hdr->duration_id == 0)		//normal mode stbc ������
	{
		tods = 2;
	}
	else
			...
			if(lsd_sync_cnt[tods] >= 1)
			{
				...
				for(i=1; i<=3; i++)
				{
					if(guide_len > lsd_head[tods][i])
						guide_len = lsd_head[tods][i];								//ȡ��ͬ��ͷ����Сֵ					
				}
				lsd_state[tods] = 1;														//ͬ�����, ����ԴMAC���ŵ�
				lsd_data_datum[tods] = guide_len - LSD_GUIDE_DATUM + LSD_DATA_OFFSET;		//��ȡ����׼����
				if(lsd_printf)
